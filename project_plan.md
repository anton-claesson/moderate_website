...
- [~] **F13.5 — SEO & social sharing.** `robots.txt` and `sitemap.xml` (`app/sitemap.ts`, reads `NEXT_PUBLIC_SITE_URL` env var) added. Still open: OG image / social card (1200×630 px banner for link previews on social media), and verifying favicon.
...
| 2026-05-03 | F13.3–F13.5 | Accessibility & performance pass. Deferred Mapbox GL JS load via IntersectionObserver (mobile TBT improvement). Added Mapbox preconnect hints. Darkened `--color-text-muted` (#6b7280→#626977) and fixed 9 WCAG AA contrast failures across footer, form labels, toggle buttons. Added `aria-label` to mobile municipality `<select>`. Added `robots.txt` and `sitemap.xml` (reads `NEXT_PUBLIC_SITE_URL`). Fixed `html[lang]` to "sv". Removed unused `VideosSection` import. | — |
